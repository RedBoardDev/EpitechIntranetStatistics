class XPHub {
    ...
    constructor() {
        this.#me = { nbXps: 0, nbXpsSoon: 0, present: [], absent: [], soon: []};
        this.#participation = {
            talk: 0,
            workshop: 0,
            hackaton: 0,
    ...
    addActivite = (title, type, status, date) => {
        ...
        switch (status) {
            case 'present':
                ...
                this.#me.present.push({ title, type, status, date });
                break;
            case 'absent':
                this.#me.nbXps -= xpLostPart;
                this.#me.absent.push({ title, type, status, date });
                break;
            case 'organisateur':
                nbOrg < limitOrg && (this.#me.nbXps += xpWinOrg) && (findAct.nbOrg += 1);
                this.#me.present.push({ title, type, status: 'organisateur', date });
                break;
            case 'soon':
                this.#me.soon.push({ title, type, status: 'inscrit', date });
                break;
            default:
                break;
        }
    ...
    countXPValidated = () => {
        this.#me.present.forEach(element => {
            switch (element.type) {
                case 'Talk':
